...
class TestFailureHandling(unittest.TestCase):
    ...
    def test_leader_failure_triggers_election(self):
        ...
        node = Node("n2", 5002, self.all_nodes)
...
class TestTransactionExecution(unittest.TestCase):
    ...
    def setUp(self):
        self.node = Node("n1", 5001, {"n1": ("localhost", 5001)})
    ...
    def test_sequential_execution(self):
        ...
        self.node.paxos_state.accept_log = [
            LogEntry(2, tx3, False, "C"),  # Out of order
            LogEntry(0, tx1, False, "C"),
            LogEntry(1, tx2, False, "C")
        ]
        self.node.paxos_state.last_executed_seq = -1
        
        # Execute transactions in order
        for expected_seq in [0, 1, 2]:
            # Find next transaction to execute
            next_seq = self.node.paxos_state.last_executed_seq + 1
            entry_to_execute = None
            
            for entry in self.node.paxos_state.accept_log:
                if entry.seq_num == next_seq and entry.status == "C":
                    entry_to_execute = entry
                    break
            
            if entry_to_execute:
                success = self.node._execute_transaction(entry_to_execute)
                self.assertTrue(success)
                entry_to_execute.status = "E"
                self.node.paxos_state.last_executed_seq = next_seq
        
        # Check final balances
        self.assertEqual(self.node.balances["A"], 5)   # 10 - 5
        self.assertEqual(self.node.balances["B"], 12)  # 10 + 5 - 3
        self.assertEqual(self.node.balances["C"], 11)  # 10 + 3 - 2
        self.assertEqual(self.node.balances["D"], 12)  # 10 + 2
    ...
    def test_transaction_with_insufficient_funds(self):
        ...
        result = self.node._execute_transaction(entry)
        
        self.assertFalse(result)
        # Balances should be unchanged
        self.assertEqual(self.node.balances["A"], 10)
        self.assertEqual(self.node.balances["B"], 10)
    ...
    def test_noop_execution(self):
        ...
        for client in ['A', 'B', 'C', 'D', 'E']:
            self.assertEqual(self.node.balances[client], 10)
